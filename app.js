...
let gamePhase = 'WAITING'; // WAITING, HIDING, HUNTING
let zoneTimerCountdown = 300; // 5 minutes (300 sec) fixe pour les paliers de zone
...
function enterWaitingRoom() {
  ...
  // Enregistrer les infos du joueur sur Firebase
...
function listenToWaitingRoom() {
  db.ref(`rooms/${roomCode}/players`).on('value', (snapshot) => {
    ...
    // Si les 2 joueurs sont là et prêts, le créateur lance la phase de cachette avec horloge UTC Firebase
  ...
  // Écouter le démarrage du jeu par le serveur Firebase
...
function listenToGameSync() {
  ...
  db.ref(`rooms/${roomCode}/gameState`).on('value', (snapshot) => {
    ...
    if (state) {
      gamePhase = state.phase;
      runGameLoopTimer(state);
    }
  });
}

function updateOpponentMarker(data) {
  if (!opponentPos) return;
  // Optionnel : afficher le chercheur ou le caché sur la carte
}
...
// --- RÈGLES DE ZONE & CHRONO STRICTS ---
function runGameLoopTimer(state) {
  setInterval(() => {
    const now = Date.now();

    if (state.phase === 'HIDING') {
      const timeLeft = Math.max(0, Math.floor((state.hidingEndTime - now) / 1000));
      const mins = String(Math.floor(timeLeft / 60)).padStart(2, '0');
      const secs = String(timeLeft % 60).padStart(2, '0');
      timerDisplay.innerText = `${mins}:${secs}`;
      statusText.innerText = "🥷 Phase de cachette en cours...";

      if (timeLeft <= 0 && userRole === 'hider' && opponentPos) {
        generateRandomCircle(opponentPos, 500);
      }
      if (timeLeft <= 0) {
      ...
      const zoneTimeLeft = Math.max(0, Math.floor((state.zoneTimerEnd - now) / 1000));
      const mins = String(Math.floor(zoneTimeLeft / 60)).padStart(2, '0');
      const secs = String(zoneTimeLeft % 60).padStart(2, '0');
      timerDisplay.innerText = `${mins}:${secs}`;
      statusText.innerText = `Traque en cours (Zone : ${currentRadius}m)`;

      // Si le palier de 5 min est atteint par le chercheur (s'il est dedans)
      if (zoneTimeLeft <= 0 && userRole === 'hider') {
        ...
        // Relance exacte à 5 minutes pour le palier suivant
...
// GESTION DU CERCLE & POSITION DU CHERCHEUR
...
function checkGameRules() {
  ...
  if (userRole === 'seeker') {
    if (distance <= currentRadius) {
      setVisualAlert('ok', 'Tu es DANS la zone !');
    } else {
      setVisualAlert('critical', 'CRITIQUE : Tu es HORS de la zone !');
      // Règle stricte : Si le chercheur sort, la zone s'agrandit pour l'aider et le chrono est remis à 5 min
...
function setVisualAlert(type, msg) {
