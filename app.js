...
function updateMarker() {
  ...
  // PETIT POINT BLANC AVEC HALO COLORÉ
...
function listenSync() {
  ...
  db.ref(`rooms/${roomCode}/roundStatus`).on('value', (s) => {
    ...
    if (status === 'SEEKER_CLAIMED' && userRole === 'hider') {
      ...
      document.getElementById('status-text').innerText = "Le chercheur t'a trouvé ! Confirme :";
...
document.getElementById('challenge-photo-input').addEventListener('change', (e) => {
  ...
  if (file && activeChallenge) {
    if (Date.now() > activeChallenge.endTime) {
      alert("⏰ Temps écoulé pour ce défi !");
      return;
    }
...
window.reviewPhoto = function(photoKey, accept, pts) {
  if (accept) {
    ...
    alert(`Photo validée ! +${pts} points attribués.`);
  } else {
    alert("Photo refusée.");
  }
...
document.getElementById('btn-finish-review').addEventListener('click', () => {
  alert("Manche officiellement terminée !");
  location.reload();
});

// BOUCLE PRINCIPALE DE JEU & MINUTEURS
function gameLoop() {
  const now = Date.now();

  // DÉCOMPTE TEMPS DE CACHETTE INITIAL
  if (gameStartTime) {
  ...
  // DEFI AUTOMATIQUE (Toutes les 20 min)
